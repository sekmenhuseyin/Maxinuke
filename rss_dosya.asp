...
SQL = "Select * from DOWNLOADS where onay=true"
dosya.open SQL,Connection,1,3
%>
<rss version="2.0">
  <channel>
    <title><%=sett("site_isim")%> Dosyalar</title>
    <link><%=sett("site_adres")%>/programs.asp?action=cats</link>
...
rssql = "select * from DOWN_CATS where cid="&dosya("cid")&""
...
%>
    <item>
      <title><![CDATA[<%=temizle(dosya("p_name"))%>]]></title>
      <link><%=sett("site_adres")%>/programs.asp?action=p_details&amp;pid=<%=dosya("pid")%></link>
      <description><%=""&temizle(Server.HTMLEncode(Left(dosya("p_info"),200)))&" . . ."%></description>
	  <author><![CDATA[<%=temizle(dosya("ekleyen"))%>]]></author>
	  <category><![CDATA[<%=temizle(hangi_kat("c_name"))%>]]></category>
  	  <pubDate><%=day(tam)%> May <%=year(tam)%> <%=right(tam,11)%> GMT</pubDate>
	  <guid><%=sett("site_adres")%>/programs.asp?action=p_details&amp;pid=<%=dosya("pid")%></guid>
	</item>
<%dosya.movenext : next%>
  </channel>
</rss>
