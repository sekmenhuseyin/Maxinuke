...
SQL = "Select * from f_mesajlar"
dosya.open SQL,Connection,1,3
%>
<rss version="2.0">
  <channel>
    <title><%=sett("site_isim")%> Forumları</title>
    <link><%=sett("site_adres")%>/forum.asp</link>
...
rssql = "select * from f_kategoriler where kategoriid="&dosya("kategoriid")&""
...
%>
    <item>
      ...
      <title><![CDATA[<%=temizle(hangi_kat("katad"))%>]]></title>
	  <link><%=sett("site_adres")%>/forum.asp?action=Control&amp;id=<%=hangi_kat("kategoriid")%></link>
      <description><%=""&temizle(Server.HTMLEncode(Left(dosya("mmsg"),200)))&" . . ."%></description>
	  <author><![CDATA[<%=temizle(dosya("yazan"))%>]]></author>
	  <author><![CDATA[<%=temizle(sett("site_isim"))%>]]></author>
	  <category><![CDATA[<%=temizle(hangi_kat("katad"))%>]]></category>
  	  <pubDate><%=day(tam)%> May <%=year(tam)%> <%=right(tam,11)%> GMT</pubDate>
	  <guid><%=sett("site_adres")%>/articles.asp?action=p_details&amp;aid=<%=dosya("mesajid")%></guid>
	</item>
<%dosya.movenext : next%>
  </channel>
</rss>
