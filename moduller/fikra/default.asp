...
kategoriler.open "Select * FROM fikra_cat ORDER BY cat_name ASC",Connection,3,3
	...
	response.Write "<CENTER><BR><BR>Veritabaninda Kayitli Fýkra yada Fýkra kategorisi bulunamadi.<BR>Lutfen daha sonra tekrar deneyin.<BR><BR><BR></CENTER>"
	End if
...
<%IF fikra_son9.Eof Then%>
	<tr>
		<td align="center"><BR><b>!</b> Bu Kategoride Kayýtlý Fýkra Yok<BR><BR></td>
	</tr>
<%ELSE%>
	<tr>
		<td align="center"><B><%=kategoriler("cat_name")%></B> kategorisi altýnda <B><%=fikra_son9.RecordCount%></B> adet fýkra bulunmakta ve bu fýkralar <B><%=n_okus%></B> kez okundular.<hr>
...
IF I >= 37 Then
%>
<a href="?mi=<%=Request.QueryString("mi")%>&action=fikra&id=<%=kategoriler("cat_id")%>"><button class="submit" style="width=100%" onclick="parentElement.click();"><%=kategoriler("cat_name")%> kategorisi altýnda yayýnlanan tüm fýkralarý göster</button></a>
<%
End IF
		End IF
		%>
...
</table><b>Toplam Fýkra : </b><%=fo_katici.RecordCount%>
<%
fo_katici.Close
...
<center>
Bu Fýkra <%=rs("hit")%> defa okundu  <%=comments("count")%> defa yorumlandý.
	<hr size="1" color="<%=tablo_cerceve%>">
	<a href="comments.asp?page=fo&action=new_comment&id=<%=id%>">Yorum Yaz</a> <b>|</b>
	<a href="comments.asp?action=comments&page=fo&id=<%=id%>">Yorumlari Oku</a> <b>|</b> <b>|</b> <a href="?mi=<%=Request.QueryString("mi")%>&action=SendFriend&ST=1&id=<%=id%>"><img src="images/temalar/<%=Session("SiteTheme")%>/sendfriend.gif" border="0" align="absmiddle"> Bu Fýkrayý Arkadaþýna Gönder ]</a> <b>]</b>
	<hr size="1" color="<%=tablo_cerceve%>">
...
Response.Write WriteMsg("Bu Fýkraya önceden puan vermiþsiniz...")
ELSE
...
%>
              <table border="0" cellpadding="2" cellspacing="0" width="100%" style="<%=TableBorder%>">
                <tr> 
                  <td class="td_menu" background="images/temalar/<%=Session("SiteTheme")%>/menu_bg.gif" height="25"><CENTER><B>Bu Fýkrayý Arkadaþýna Gönder</B></CENTER></td>
                </tr>
                <tr> 
